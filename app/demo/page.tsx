...
import { DEMO_PROFILE, DEMO_ORDERS, DEMO_STATS } from '@/lib/demo-data'
import { DemoShareLink } from '@/components/demo/DemoNav'

export default function DemoDashboardPage() {
  const totalOrders = DEMO_ORDERS.length
  const completedCount = DEMO_STATS.completedOrders
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2">
        <div>
          ...
          <p className="text-text-secondary text-sm">
            Here&apos;s what&apos;s happening with your deals, <span className="font-semibold text-text-primary">@{DEMO_PROFILE.username}</span>.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="relative group overflow-hidden bg-brand-success rounded-3xl p-6 shadow-elevation-low hover:shadow-elevation-medium transition-shadow">
          ...
          <div className="relative z-10">
            <span className="text-[14px] font-medium text-white/90">Total Earnings</span>
          </div>
          <div className="relative z-10 mt-2">
            <div className="text-4xl font-bold text-white tracking-tight mb-2">GHS {(DEMO_STATS.totalRevenue / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
        ...
        <div className="relative group bg-surface-card border border-border rounded-3xl p-6 shadow-elevation-low hover:shadow-elevation-medium transition-shadow">
          <span className="text-[14px] font-medium text-text-primary">Active Orders</span>
          <div className="mt-2">
            <div className="text-4xl font-bold text-text-primary tracking-tight mb-2">{DEMO_STATS.activeOrders}</div>
        ...
        {/* Completed */}
        <div className="relative group bg-surface-card border border-border rounded-3xl p-6 shadow-elevation-low hover:shadow-elevation-medium transition-shadow">
          <span className="text-[14px] font-medium text-text-primary">Completed</span>
          <div className="mt-2">
            <div className="text-4xl font-bold text-text-primary tracking-tight mb-2">{DEMO_STATS.completedOrders}</div>
            <p className="text-[11px] text-text-muted font-medium inline-flex items-center gap-1.5 p-1 rounded-md bg-surface-light">
        ...
        {/* Profile Views */}
        <div className="relative group bg-surface-card border border-border rounded-3xl p-6 shadow-elevation-low hover:shadow-elevation-medium transition-shadow">
          <span className="text-[14px] font-medium text-text-primary">Profile Views</span>
          <div className="mt-2">
            <div className="text-4xl font-bold text-text-primary tracking-tight mb-2">{DEMO_STATS.profileViews}</div>
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        ...
        <div className="lg:col-span-8 bg-surface-card border border-border rounded-3xl shadow-elevation-low p-6 overflow-hidden">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-[16px] font-bold text-text-primary">Recent Orders</h2>
            <Link
              href="/demo/orders"
