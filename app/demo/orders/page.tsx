        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-border">
              {orders.map((order) => (
                <tr key={order.id} className="group hover:bg-brand-success/[0.03] transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <span className="text-[13px] text-text-secondary font-medium">
                      {order.packages?.title || 'Custom Service'}
                    </span>
                  </td>
                  <td className="px-5 py-4">
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-text-muted">{order.packages?.title || 'Custom Service'}</div>
                  </div>
                </div>
                <span className={`apple-badge border ${getStatusStyle(order.order_status)}`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${getStatusDot(order.order_status)}`} />
